...
class hostadimnForm(forms.Form):
    ...
    hostlist = forms.ModelChoiceField(queryset=hostinfo.objects.all(), empty_label="*", to_field_name="hostname",
                                      widget=forms.Select(
                                          attrs={'id': 'hostlist', 'class': 'selectpicker show-tick form-control',
                                                 'name': 'hostlist'}))
