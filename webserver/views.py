...
def loginZabbix():
    ...
    zabbix_url = "http://192.168.36.133/zabbix/api_jsonrpc.php"
...
def getfsfree(*args, **b):
    ...
    disk_data = json.dumps({
        ...
        "params": {
            ...
            "hostids": b["parameter"],
            "search": {
                "key_": "vfs.fs.size[/,free]"  # 只查询key_包含 "vfs.fs.size[/,free]的item
            }
        },
        "auth": b["session"],
        "id": 2
...
def getfshomefree(*args, **b):
    ...
    disk_data = json.dumps({
        ...
        "params": {
            ...
            "hostids": b["parameter"],
            "search": {
                "key_": "vfs.fs.size[/home,free]"  # 只查询磁盘的homefree
            }
        },
        "auth": b["session"],
        "id": 2
...
def getfsbootfree(*args, **b):
    ...
    diskboot_data = json.dumps({
        ...
        "params": {
            ...
            "hostids": b["parameter"],
            "search": {
                "key_": "vfs.fs.size[/boot,free]"  # 只查询磁盘的homefree
            }
        },
        "auth": b["session"],
        "id": 2
...
def getmemsize(*args, **b):
    ...
    mem_data = json.dumps({
        ...
        "params": {
            ...
            "hostids": b["parameter"],
            "search": {
                "key_": "vm.memory.size[available]"  # 只查询key_包含“vm.memory.size[available]”字段的item
            }
        },
        "auth": b["session"],
        "id": 2
...
def getCpuUserHistory(*args, **b):
    ...
    用户空间使用CPU比例
    '''
    CpuUserHis_data = json.dumps({
        "jsonrpc": "2.0",
        "method": "history.get",
        "params": {
            "output": "extend",
            # "itemids": b["parameter"],
            "itemids": 28364,  # system.cpu.util[,user]  用户空间使用CPU比例
            "history": 0,
            "limit": 16
            # "search": {
            #     "key_": "vm.memory.size[available]",  # 只查询key_包含“vm.memory.size[available]”字段的item
...
def getCpuSysHistory(*args, **b):
    ...
    内核空间使用CPU比例
    '''
    CpuSysHis_data = json.dumps({
        "jsonrpc": "2.0",
        "method": "history.get",
        "params": {
            "output": "extend",
            # "itemids": b["parameter"],
            "itemids": 28363,  # system.cpu.util[,system]  内核空间使用CPU比例
            "history": 0,
            "limit": 16
            # "search": {
            #     "key_": "vm.memory.size[available]",  # 只查询key_包含“vm.memory.size[available]”字段的item
...
def getNetInDataTraffic(*args, **b):
    ...
    网络流入数据
    '''
    NetDataTraffic_data = json.dumps({
        "jsonrpc": "2.0",
        "method": "history.get",
        "params": {
            "output": "extend",
...
def sendemail():
    ...
    邮件发送
    '''
    mail_host = "smtp.163.com"  # 这里先设置smtp服务器，就像：smtp.163.com
    mail_user = "1300042631@qq.com"  # 用户名
    mail_pass = "1300042631"  # 密码

    sender = '1300042631@qq.com'  # 发送邮件
    receivers = '1300042631@qq.com'  # 接收邮件
